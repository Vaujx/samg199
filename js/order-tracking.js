...
// Declare CONFIG and getBinData (assuming they are globally available or imported elsewhere)
const CONFIG = window.CONFIG || {} // Initialize CONFIG as an empty object if it doesn't exist
const getBinData =
  window.getBinData ||
  (() => {
    // Initialize getBinData as a dummy function
    return Promise.reject("getBinData is not defined. Ensure jsonbin-api.js is loaded.")
  })
...
async function trackOrders() {
  ...
  try {
    ...
    try {
      console.log("Fetching orders from ORDER_TRACKING bin...")
